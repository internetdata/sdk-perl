...
Every endpoint is licensed, so a key is required rather than optional: there is no anonymous tier to fall back to. Create one in the console and give it the `db.download` scope.
